...
import { $createEmojiNode, EmojiNode } from '../nodes/EmojiNode.jsx';
...
function $findAndTransformEmoji(node) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (emojiData !== undefined) {
      ...
      const matchLength = emojis.get(text.slice(i, i + 2)) ? 2 : 1;

      if (i === 0) {
        [targetNode] = node.splitText(matchLength);
      } else {
        [, targetNode] = node.splitText(i);
...
// Export the emoji mappings for use in other components
export { emojis };
